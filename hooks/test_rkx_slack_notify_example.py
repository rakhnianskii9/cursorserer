...
import rkx_slack_notify_example as notify  # noqa: E402
...
BLOCKED_FIXTURE = {
    ...
    "run_id": "2026-08-07-rkx-provider-options",
    "wave": "wave-2",
    "kind": "waiting_user",
    "notification_type": "attention",
    "problem_title": "Operators are missing from the configuration list at step 3",
    "summary": (
        "The service shows an empty list because the saved workspace "
        "settings did not return any operators."
    ),
    "blocker": "The check received HTTP 401 because authentication was missing.",
    "next_action": (
...
class SlackNotificationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.artifact_path = (
            self.repo_root
            / "loops"
            / "2026-08-07-rkx-provider-options"
    ...
    def test_blocked_card_is_human_readable_and_has_no_dead_actions(self) -> None:
        ...
        self.assertIn(
            "Operators are missing from the configuration list at step 3",
            card["text"],
        )
        self.assertIn(
            "Blocker: The check received HTTP 401 because authentication was missing.",
        ...
        self.assertIn("User action:", card["text"])
    ...
    def test_completed_card_is_a_result(self) -> None:
        artifact = {
            **BLOCKED_FIXTURE,
            ...
            "summary": "The loop completed and the result was saved.",
            "blocker": "",
            "next_action": "",
        }
        ...
        self.assertIsNotNone(card)
        assert card is not None
        self.assertIn("✅ Work result:", card["text"])
        self.assertIn("The loop completed and the result was saved.", card["text"])
        self.assertNotIn("User action:", card["text"])
    ...
    def test_invalid_notification_type_for_kind_is_rejected(self) -> None:
        self.write_artifact(
    ...
    def test_urls_with_embedded_secrets_are_not_rendered(self) -> None:
        artifact = {
            **BLOCKED_FIXTURE,
            "full_verdict_available": True,
            "full_verdict_url": (
                "https://cursor.com/agents/example?token=<redacted>"
            ),
        }
        ...
        self.assertIn("Full result: available in Cursor", card["text"])
