...
RUN / WAVE
DECISION_ID + DECISION_KIND: NEXT_WAVE_SPEC | END | HARD_BLOCKER
...
DECISION_KIND: NEXT_WAVE_SPEC | END | HARD_BLOCKER
RECOVERY_ATTEMPTED: false
...
- `HOLE` → `SINGLE_NEXT_CHECK` is mandatory and falsifiable. On the hard path
  the check must advance recorded `success_criteria` (or the access required
  for it). Forbidden: reframing the dig onto a title keyword when
  `title_only_scope` is false and `product_goal_met` remains no. Soft-path
  `HOLE` may flag Chat/decision that closed the title but not the product PASS.
