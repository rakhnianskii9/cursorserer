...
1. **Reload Window** (once after `${CONTROL_PLANE_ROOT}/commands/` appears).
2. Chat → mode **Agent**.
...
4. Delegation: the primary agent selects an internal role from
   `${CONTROL_PLANE_ROOT}/agents/`; this is not a slash interface.

`rkx-loop-core` and phase skills are internal implementation details, not
commands for manual selection.

`/rkx-loop` belongs only to `${CONTROL_PLANE_ROOT}/commands/`. The
`rkx-loop-orchestrator` agent (file `${CONTROL_PLANE_ROOT}/agents/rkx-loop.md`) is an
...
one verified checker fact-slot for preflight and saves only the matching
...
Before each fan-out, the Orchestrator invokes and awaits one token-mode
`fact-slot` in the `PREFLIGHT` group. the verified checker collects source/tool/MCP/browser
...
- stop-hook `${CONTROL_PLANE_ROOT}/hooks/rkx-slack-notify.sh` sends only two run-scoped cards
...
Canonical schema and ownership: `${CONTROL_PLANE_ROOT}/skills/rkx-loop-core/SKILL.md`.
Secrets belong only in `${HOME}/.cursor/rkx-slack-notify.env`. Push to a phone = a
Slack app.
...
| What | Path |
|---|---|
| Slash commands | `${CONTROL_PLANE_ROOT}/commands/*.md` |
| Skills | `${CONTROL_PLANE_ROOT}/skills/rkx-loop-*/SKILL.md` |
| Delegation agents | `${CONTROL_PLANE_ROOT}/agents/*.md` |
| Rules | `${CONTROL_PLANE_ROOT}/rules/rkx-*.mdc` (+ `token-economy`, `normal-agent-workflow`, …) |
| Models | `${CONTROL_PLANE_ROOT}/CURSOR-MODELS.md` |
| Reference catalogs | `${CONTROL_PLANE_ROOT}/reference/blueprint-index.yaml`, `${CONTROL_PLANE_ROOT}/reference/system-design-primer/`, `${CONTROL_PLANE_ROOT}/reference/telephony/catalog.yaml` |
| Gate / optional runs | `${CONTROL_PLANE_ROOT}/rules/infrastructure-core.mdc` + `rkx-always-on.mdc`, `loops/` |
...
After installation, commands live under the control-plane root's
`${CONTROL_PLANE_ROOT}/commands/` directory. Cursor's `/` menu reads **commands** and
**skills**; prompt archives are not part of this public package.
