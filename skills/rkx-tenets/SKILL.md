...
**Observed timings (order-of-magnitude):** `fast` on a narrow package path — seconds; `fast` on the full repo — often tens of seconds; `balanced` on the full repo — minutes / timeout risk. Sub-second claims apply only to a **narrow** path.
...
- **Concurrency limit: ≤6 simultaneous `rank_files`; serialize full-repo runs.**
...
Time = **path × mode**, not an average “~500ms”.

| Scope | path | mode | top_n |
|-------|------|------|-------|
| Module known | `<package-or-infra-path>/…` | `fast` | 8–12 |
| Module unknown — initial discovery | top-level packages root (not the repo root) | `fast` | 10–15 |
...
**Path rules:** `include_patterns` does **not** replace a narrow `path` (the scanner still walks the tree). If full-repo returns `files: []`, retry with a known package path; do not repeat full-repo. If an infrastructure path is external to the workspace, use the repository's runtime configuration or prompt instead of assuming a host path.

## Tool Matrix

| Intent | Tool | When |
|--------|------|------|
| Fast file list | `tenets_rank_files` | Unknown scope: narrow package path + `mode=fast`; full repo as a last resort; do not batch with CodeGraph in one turn |
...
2. tenets_session action=pin_folder path=<active-module-path>
...
- Use each package's declared UI stack; do not mix systems
- Persistence changes → migration + registry
