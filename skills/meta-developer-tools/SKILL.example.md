...
Pair with an optional workspace sync-lab skill for sync pipeline experiments; security-guidance for
  tokens/webhooks. Not for browser DevTools or IDE diagnostics.
user-invocable: true
...
- Env: `FACEBOOK_APP_ID`, `FACEBOOK_APP_SECRET`, `FACEBOOK_WEBHOOK_VERIFY_TOKEN` (+ legacy aliases)
- Webhooks/OAuth: `${WORKSPACE_ROOT}/server/routes/provider.route.ts`,
  `${WORKSPACE_ROOT}/server/services/provider-subscriptions.ts`,
  `${WORKSPACE_ROOT}/scripts/subscribe-webhooks.ts`
- Marketing/rate: `${WORKSPACE_ROOT}/server/services/provider-marketing.ts`
- Doc: `${WORKSPACE_ROOT}/docs/provider-integrations.md`
- Sync experiments → an explicitly enabled workspace sync-lab skill, not this skill alone
